...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ###########################################################################
    # DONE: Implement a single forward step for the vanilla RNN. Store the    #
    # next hidden state and any values you need for the backward pass in the  #
    # next_h and cache variables respectively.                                #
    ###########################################################################
    # next_hidden_state = tanh (W_hh * h[t-1] + W_xh * x[t] + b)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(0, T):
        ...
        prev_h, cache_step = rnn_step_forward(x=x[:, i, :],
                                              prev_h=prev_h,
                                              Wx=Wx,
                                              Wh=Wh,
                                              b=b)
...
def rnn_backward(dh, cache):
    ...
    # sequence of data. You should use the rnn_step_backward function that    #
    # you defined above.                                                      #
    ###########################################################################
    # get matrix dimension
    N, T, H = dh.shape
    ...
    for i in range(T-1, -1, -1):
        id_str = str(i)
        cache_name = 'cache' + id_str
        cache_step = cache[cache_name]

        # gradient from upstream = dh[:, i, :] ~ from output +
        #                          dh0 ~ from the upstream layer
        dx_step, dh0, dWx_step, dWh_step, db_step = \
            rnn_step_backward(dnext_h=dh[:, i, :] + dh0,
                              cache=cache_step)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    input sequence composed of T vectors, each of dimension D. The LSTM uses a
    hidden size of H, and we work over a minibatch containing N sequences.
...
def lstm_backward(dh, cache):
    ...
    dx, dh0, dWx, dWh, db = None, None, None, None, None
    ###########################################################################
    # TODO: Implement the backward pass for an LSTM over an entire            #
...
def temporal_affine_forward(x, w, b):
    ...
    length T. We use an affine function to transform each of those vectors
    into a new vector of dimension M.
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    range 0 <= y[i, t] < V
    - mask: Boolean array of shape (N, T) where mask[i, t] tells whether or
    not the scores at x[i, t] should contribute to the loss.
