...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #############################################################################
    # XXX: Implement the test-time forward pass for batch normalization. Use   #
...
def batchnorm_backward(dout, cache):
  ...
  dsample_mean = np.sum(dx_hat * (-1/np.sqrt(sample_var + eps)) , axis=0) + dsample_var * ((np.sum(-2*(x-sample_mean))) / m)
...
def max_pool_forward_naive(x, pool_param):
  ...
  for n in xrange(N):
    for c in xrange(C):
      for i in xrange(H1):
        for j in xrange(W1):
          x_window = x[n, c, i * stride : i * stride + pool_height, j * stride : j * stride + pool_width]
